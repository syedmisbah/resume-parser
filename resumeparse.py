...
class resumeparse(object):
    ...
    print("Imported function script")

    #Reading required constant files
    pincodes = pd.read_csv('Pincode_30052019.csv',usecols=['StateName','Pincode','Division Name'])
    universities_list = pd.read_csv('list_universities.csv')
        ...
        for i, line in enumerate(string_to_search):
            ...
            if [o for o in resumeparse.objective if header.startswith(o)]:
                try:
                    ...
                    header = [o for o in resumeparse.objective if header.startswith(o)][
                        0
                    ]
                ...
                try:
                    ...
                    header = [
                        w
                        for w in resumeparse.work_and_employment
                        if header.startswith(w)
                    ][0]
                    resume_segments["work_and_employment"][header] = i
            elif [
                e for e in resumeparse.education_and_training if header.startswith(e)
            ]:
                try:
                ...
                try:
                    ...
                    resume_indices.append(i)
                    header = [
                        s for s in resumeparse.skills_header if header.startswith(s)
                    ][0]
    ...
    def extract_university(text, file):
        ...
        if(len(college_name)!=0):
            ...
            regex = r"(?:\S+\s)?\S*university\S*(?:\s\S+)?"
            matches = re.finditer(regex, text, re.MULTILINE | re.IGNORECASE)
            matches_list = []
            for match in matches:
                mm = match.group(0)
                matches_list.append(mm)
            return matches_list
               

    def job_designation(text):
        job_titles = []
        finder = Finder()
        try:
            matches = finder.findall(text)
            return [each_item.match for each_item in matches]
        except:
    ...
    def extract_address(text):
        NA_ADDR_TEXT = 'Address Not Provided'
        regex = r"[0-9]{6}|[0-9]{3}\s[0-9]{3}"
        list_pin = [int("".join(x.split())) for x in re.findall(regex,text)]
        # print(list_pin)
        if(len(list_pin))!=0:
            for each_code in list_pin:
                # print("In loop for ",each_code)
                try:
                    # print("trying to search")
                    # print(pincodes.loc[pincodes['Pincode']==each_code,'Pincode'])
                    pin_code = resumeparse.pincodes.loc[resumeparse.pincodes['Pincode']==each_code,'Pincode'].values[0]
                    state_name = resumeparse.pincodes.loc[resumeparse.pincodes['Pincode']==each_code,'StateName'].values[0]
                    division_name = resumeparse.pincodes.loc[resumeparse.pincodes['Pincode']==each_code,'Division Name'].values[0]
                    # print(pin_code,state_name,division_name)
                    return pin_code,state_name,division_name
                    break
                except: 
                    # print("Nothing found ")
                    return NA_ADDR_TEXT,NA_ADDR_TEXT,NA_ADDR_TEXT
                    pass
        else:
    ...
    def extract_years_of_exp(text):    
        regex = r"\b(?:\d+-\d+ [yY]ears|[02-9] [Yy]ears|[02-9][\+] [Yy]ears|1 [Yy]ear|[1-9]\d+\+? [Yy]ears)\b"
        matches = re.finditer(regex, text, re.MULTILINE | re.IGNORECASE)
        matches_list = []
        for match in matches:
            mm = match.group(0)
        ...
        if len(matches_list)!= 0:
            return [[int(s) for s in re.findall(r'\b\d+\b', each_match)][0] for each_match in matches_list][0]
        else:
            return 0

    def extract_keywords(text):
        stop_words = ('stopwords-en.txt')
        rake_obj = RAKE.Rake(stop_words=stop_words)
        keywords = [x[0] for x in rake_obj.run(text)[0:15]] 
        return keywords
    ...
    def read_file(file):
        ...
        designation = resumeparse.job_designation(full_text)

        degree = resumeparse.get_degree(full_text)
        company_working = resumeparse.get_company_working(full_text)
        pin_code,state_name,division_name = resumeparse.extract_address(full_text)
        years_exp = resumeparse.extract_years_of_exp(full_text)

        keywords = resumeparse.extract_keywords(full_text)
